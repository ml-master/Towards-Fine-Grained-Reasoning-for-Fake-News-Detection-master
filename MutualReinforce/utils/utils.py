...
def load_tweets(dataset):
    return torch.load(osp.join(get_root_dir(),
                                   f"{dataset}_tweets.pt"))


def save_tweets(dataset, all_tweets_d, all_replies_d, all_tweets_score_d):
    torch.save((all_tweets_d, all_replies_d, all_tweets_score_d), osp.join(get_root_dir(),
                                                                           f"{dataset}_tweets.pt"))
...
def save_labels(dataset, labels_d):
    torch.save(labels_d, osp.join(get_root_dir(),
                                          f"{dataset}_labels.pt"))
...
def read_tweets_and_scores(dataset):
    import torch
    all_tweets_d, all_replies_d, all_tweets_score_d = torch.load(osp.join(get_root_dir(),
                                                                          f"{dataset}_tweets.pt"))
...
def read_news_articles_labels(dataset_name="politifact", n_samples=0):
    ...
    if n_samples > 0:
        news_article_df = pd.read_csv(config.FAKE_NEWS_DATA + f"\\{dataset_name}_news_articles.tsv", sep='\t',
...
def only_directories(path):
    return [name for name in os.listdir('/data1/liangbn/code/FinerFact/MutualReinforce/fake_news_data') if osp.isdir(osp.join(path, name))]


def get_data_list():
    #politifact_fake = only_directories(config.POLITIFACT_FAKE)
    #politifact_real = only_directories(config.POLITIFACT_REAL)
    gossipcop_fake = only_directories(config.GOSSIPCOP_FAKE)
    gossipcop_real = only_directories(config.GOSSIPCOP_REAL)
...
def get_dataset_names(dataset):
    if dataset == "politifact":
        dataset_names = {
            config.POLITIFACT_NAME: config.POLITIFACT_DATASET_NAMES
        }
    elif dataset == "gossipcop":
        dataset_names = {
            config.GOSSIPCOP_NAME: config.GOSSIPCOP_DATASET_NAMES
        }
    elif dataset == "both":
