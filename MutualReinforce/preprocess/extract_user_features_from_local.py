...
def crawl_and_extract_user_features(api, user_ids, output_dir, example_name, global_user_features_d):
    ...
    for user_id in user_ids:
        ...
        try:
            ...
            raw_user_json = api.get_user(user_id=user_id)
...
def crawl_and_extract_user_features_batch(api, user_ids, output_dir, example_name, global_user_features_d, global_user_features_new_d, old_user_df):
    all_user_features = []
    n_user_feature_lost, n_user_feature_from_original_data = 0, 0
...
def main():
    ...
    for path in [POLITIFACT_REAL,  POLITIFACT_FAKE, POLITIFACT_REAL, GOSSIPCOP_REAL, GOSSIPCOP_FAKE]:
        f = open("/data1/liangbn/code/FinerFact/MutualReinforce/fake_news_data/gossipcop_real/all_user_ids_and_features.pkl", "rb")    #无all_user_ids_and_features.pkl
        ...
        for filename, user_ids in global_user_ids.items():
            ...
            new_user_df_fullname = path + "/" + filename+'/new_user.tsv'
            ...
            if args.use_batch:
                n_user_feature_lost, n_user_feature_from_original_data, n_total = crawl_and_extract_user_features_batch(api, user_ids, path, filename, global_user_features_d, global_user_features_new_d, old_user_df)

            else:
                n_user_feature_lost, n_user_feature_from_original_data, n_total = crawl_and_extract_user_features(api, user_ids, path, filename, global_user_features_d)
        ...
        # f = open(path + "/new_all_user_features.pkl", "wb")    #先不考虑
